    ...
    try:
        ...
        for row in readCSV:
                ...
                for lineaA in resultQueryArea:
                    if area['area'].strip(' ').decode('latin_1').encode('utf_8')==lineaA.area:#.decode('latin_1').encode('utf_8')
                        idArea=lineaA.id
                        break
...
#probando insertar SubArea
                        #busca la nacionalidad que es igual en el fichero csv e inserta el record completo
                for linea in resultQueryArea:
                    if row[5].encode('Latin_1').encode('utf-8')==lineaA.area:#.decode('latin_1').encode('utf_8')
                        idArea=linea.id

                        subarea ={'nombre':row[5].strip(' '),'id_Area':idArea,}
                        registroS = db.sub_area(**db.sub_area._filter_fields(subarea)) # Check for a match.

                        if registroS:
                             subarea_id =  registroS.id
                        else: db.sub_area.insert(**db.sub_area._filter_fields(subarea))
